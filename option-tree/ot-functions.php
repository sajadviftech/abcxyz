<?php
function viftech_bgecho($array, $setting = false) {
	if(!empty($array)) {
...
function viftech_paddingecho($array) {
	if(!empty($array)) {
...
function viftech_borderecho($array) {
	if(!empty($array)) {
		$return = '';
		$unit = array_key_exists('unit',$array) ? $array['unit'] : 'px';
...
function viftech_linkcolorecho($array, $start = '') {
	if(!empty($array)) {
...
$viftech_fontlist = array();

function viftech_google_webfont() {
		global $viftech_fontlist;
		$options = array( 
			array( 
					'option' => "primary_font", 
					'default' => ""
		...
		foreach($options as $option) {
			...
			if (!empty($array['font-family'])) { 
				if (!in_array($array['font-family'], $viftech_fontlist)) {
					if (in_array($array['font-family'], $google_fonts)) {
						$font = $array['font-family'].$weights;
						array_push($viftech_fontlist, $font);
					}
				}
			} else if ($option['default']) {
				if (!in_array($option['default'], $viftech_fontlist)) {
		...
		$font_list = array_unique($viftech_fontlist);
...
function viftech_typeecho($array, $important = false, $default = false) {
	global $viftech_fontlist;
...
function viftech_spacingecho($array, $std = false, $type = 'padding') {
	if(!empty($array)) {
		$unit = array_key_exists('unit', $array) ? $array['unit'] : 'px';
		if (array_key_exists('top', $array)) {
...
function viftech_measurementecho($array) {
	if(!empty($array)) {
		echo $array[0] . $array[1];
	}
}

function viftech_hex2rgb($hex) {
...
function viftech_adjustColorLightenDarken($color_code,$percentage_adjuster = 0) {
...
function viftech_catcolorecho($array) {
	if(!empty($array)) {
