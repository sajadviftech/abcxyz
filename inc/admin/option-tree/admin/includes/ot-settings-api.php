          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            if ( $show_buttons ) {
              ...
              echo '<div id="option-tree-settings-api">';
                ...
                $options_active_class = 'ot-theme-options' === wp_unslash($_GET['page']) ? 'active' : '';
                $settings_active_class = 'ot-settings' === wp_unslash($_GET['page']) ? 'active' : '';
                
                /* Sub Header */
                echo '<div id="option-tree-sub-header">';
                echo '<h2><a href="'. admin_url( "themes.php?page=ot-theme-options" ).'" class="'.$options_active_class.'">Theme Options</a></h2> <h2><a href="'. admin_url( "themes.php?page=ot-settings" ).'" class="'.$settings_active_class.'">Import / Export </a></h2>';
                if ( $show_buttons )
