<?php function vif_freescroll( $atts, $content = null ) {
  $atts = vc_map_get_attributes( 'vif_freescroll', $atts );
  extract( $atts );
		...
		<?php
			if ($type === 'text') {
				...
				foreach ($images as $image) {
					...
					?>
					<figure class="<?php echo esc_attr($vif_columns); ?> columns">
						<?php echo wp_get_attachment_image($image, 'full'); ?>
					</figure>
					<?php
				}
			} else if ($type === 'instagram') {
				$instagram = vif_getInstagramPhotos($number);
				
				if ($instagram) {
					foreach ($instagram as $item) {
						?>
						<div class="<?php echo esc_attr($vif_columns); ?> columns">
				...
				if ($free_posts->have_posts()) :  while ($free_posts->have_posts()) : $free_posts->the_post();
						...
						set_query_var( 'vif_cat', false );
						set_query_var( 'vif_date', true );
...
vif_add_short('vif_freescroll', 'vif_freescroll');